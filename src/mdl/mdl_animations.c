...
void matrix_multiply_3x4( float result[3][4], float parent_matrix[3][4], float local_matrix[3][4] ) {
	...
	result[0][0] = parent_matrix[0][0] * local_matrix[0][0] + parent_matrix[0][1] * local_matrix[1][0] + parent_matrix[0][2] * local_matrix[2][0];
	result[0][1] = parent_matrix[0][0] * local_matrix[0][1] + parent_matrix[0][1] * local_matrix[1][1] + parent_matrix[0][2] * local_matrix[2][1];
	result[0][2] = parent_matrix[0][0] * local_matrix[0][2] + parent_matrix[0][1] * local_matrix[1][2] + parent_matrix[0][2] * local_matrix[2][2];
	result[0][3] = parent_matrix[0][0] * local_matrix[0][3] + parent_matrix[0][1] * local_matrix[1][3] + parent_matrix[0][2] * local_matrix[2][3] + parent_matrix[0][3];

	result[1][0] = parent_matrix[1][0] * local_matrix[0][0] + parent_matrix[1][1] * local_matrix[1][0] + parent_matrix[1][2] * local_matrix[2][0];
	result[1][1] = parent_matrix[1][0] * local_matrix[0][1] + parent_matrix[1][1] * local_matrix[1][1] + parent_matrix[1][2] * local_matrix[2][1];
	result[1][2] = parent_matrix[1][0] * local_matrix[0][2] + parent_matrix[1][1] * local_matrix[1][2] + parent_matrix[1][2] * local_matrix[2][2];
	result[1][3] = parent_matrix[1][0] * local_matrix[0][3] + parent_matrix[1][1] * local_matrix[1][3] + parent_matrix[1][2] * local_matrix[2][3] + parent_matrix[1][3];

	result[2][0] = parent_matrix[2][0] * local_matrix[0][0] + parent_matrix[2][1] * local_matrix[1][0] + parent_matrix[2][2] * local_matrix[2][0];
	result[2][1] = parent_matrix[2][0] * local_matrix[0][1] + parent_matrix[2][1] * local_matrix[1][1] + parent_matrix[2][2] * local_matrix[2][1];
	result[2][2] = parent_matrix[2][0] * local_matrix[0][2] + parent_matrix[2][1] * local_matrix[1][2] + parent_matrix[2][2] * local_matrix[2][2];
	result[2][3] = parent_matrix[2][0] * local_matrix[0][3] + parent_matrix[2][1] * local_matrix[1][3] + parent_matrix[2][2] * local_matrix[2][3] + parent_matrix[2][3];
}
...
void CalcBoneQuaternion( int frame, float s, const mstudiobone_t *pbone, const mstudioanim_t *panim,
	...
	for ( int j = 0; j < 3; j++ ) {
		if ( panim->offset[j + 3] ) {
			...
			const mstudioanimvalue_t *panimvalue = (const mstudioanimvalue_t *)( animBase + panim->offset[j + 3] );
			...
			if ( panimvalue->num.valid > k ) {
				...
				angle1[j] += panimvalue[panimvalue->num.valid].value * pbone->scale[j + 3];

				if ( panimvalue->num.total > k + 1 ) {
					angle2[j] = angle1[j];
				} else {
					angle2[j] += panimvalue[panimvalue->num.valid + 2].value * pbone->scale[j + 3];
				}
			}
		}
	}
	...
	for ( int j = 0; j < 3; j++ ) {
		...
		if ( panim->offset[j] != 0 ) {
			...
			const mstudioanimvalue_t *panimvalue =
				(const mstudioanimvalue_t *)( animBase + panim->offset[j] );
			int k = frame;
...
void mdl_animation_update( mdl_animation_state_t *state, float delta_time, studiohdr_t *header, unsigned char *data, mdl_seqgroup_blob_t *seqgroups ) {
	...
	if ( delta_time > ( 1.0f / seq->fps * 2.0f ) ) { // Allow 2x slowdown max
...
void transform_vertex_by_bone( vec3_t result, vec3_t vertex, float bone_matrix[3][4] ) {
	...
	result[0] = bone_matrix[0][0] * vertex[0] + bone_matrix[0][1] * vertex[1] + bone_matrix[0][2] * vertex[2] + bone_matrix[0][3];

	result[1] = bone_matrix[1][0] * vertex[0] + bone_matrix[1][1] * vertex[1] + bone_matrix[1][2] * vertex[2] + bone_matrix[1][3];

	result[2] = bone_matrix[2][0] * vertex[0] + bone_matrix[2][1] * vertex[1] + bone_matrix[2][2] * vertex[2] + bone_matrix[2][3];
}
