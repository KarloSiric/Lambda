...
typedef struct {
    ...
    float current_frame;
    float previous_frame;  // Track previous frame for event detection
    bool  is_looping;
...
void mdl_animation_process_events(
    mdl_animation_state_t *state, studiohdr_t *header, unsigned char *data );
