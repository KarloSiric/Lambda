 ...
 *  Last Modified: 2025-09-27 16:48:36
 *----------------------------------------------------------------------
 *  Description:
 *      
 *----------------------------------------------------------------------
...
static const char* k_result_defaults[MDL_RESULT_COUNT] = {
    ...
    [MDL_ERROR_FILE_TOO_SMALL] =
        "File is too small to contain a valid MDL.",

    [MDL_ERROR_INVALID_MAGIC] =
        "Invalid MDL header magic (not IDST/IDSQ).",
    [MDL_ERROR_INVALID_VERSION] =
        "Unsupported MDL version.",
    [MDL_INFO_SEQUENCE_GROUP_FILE] =
        "This is a sequence-group MDL (animations only). Open the base model instead.",
    [MDL_INFO_TEXTURE_MODEL_FILE] =
        "This is a texture-model MDL. Use it as a companion, not as the main model.",

    [MDL_ERROR_BONE_COUNT_INVALID] =
        "Invalid bone count or exceeds limits.",
    [MDL_ERROR_BODYPART_COUNT_INVALID] =
        "Invalid bodypart count or exceeds limits.",
    [MDL_ERROR_MODEL_INDEX_INVALID] =
        "Corrupt model index/offset.",
    [MDL_ERROR_MESH_INDEX_INVALID] =
        "Corrupt mesh index/offset.",
    [MDL_ERROR_VERT_INDEX_INVALID] =
        "Corrupt vertex index/offset.",
    [MDL_ERROR_NORM_INDEX_INVALID] =
        "Corrupt normal index/offset.",
    [MDL_ERROR_TRICMD_BROKEN] =
        "Triangle command stream is malformed.",
    [MDL_ERROR_BONE_PARENT_OUT_OF_RANGE] =
        "Bone parent index is out of range.",
    [MDL_ERROR_TOO_MANY_BONES] =
        "Bone count exceeds engine limit.",
    [MDL_ERROR_TOO_MANY_VERTS] =
        "Vertex count exceeds engine limit.",

    [MDL_ERROR_MISSING_TEXTURE_FILE] =
        "Main MDL has no textures; companion texture MDL not found.",
    [MDL_ERROR_TEXTURE_FORMAT_UNSUPPORTED] =
        "Unsupported texture format (expected paletted 8bpp).",
    [MDL_ERROR_SKIN_FAMILY_OUT_OF_RANGE] =
        "Requested skin family index is out of range.",

    [MDL_ERROR_SEQGROUP_NOT_FOUND] =
...
size_t mdl_format_message(char* out, size_t cap, mdl_result_t r, const mdl_msg_ctx_t* ctx) {
    ...
    switch (r) {
        ...
        case MDL_ERROR_INVALID_MAGIC:
            return (size_t)snprintf(out, cap,
                "ERROR [%s]: '%s' is not an IDST/IDSQ MDL (invalid magic).",
                name, path);

        case MDL_ERROR_INVALID_VERSION:
            if (version >= 0) {
                return (size_t)snprintf(out, cap,
                    "ERROR [%s]: '%s' has unsupported MDL version %d.",
                    name, path, version);
            }
            break;
        ...
        case MDL_ERROR_MISSING_TEXTURE_FILE:
            return (size_t)snprintf(out, cap,
                "WARN  [%s]: '%s' has no embedded textures and companion texture MDL is missing.",
                name, path);

        case MDL_ERROR_BONE_PARENT_OUT_OF_RANGE:
            return (size_t)snprintf(out, cap,
                "ERROR [%s]: '%s' bone parent index out of range (got=%d, max=%d).",
                name, path, got, expected);
    ...
    // Fallback generic
    return (size_t)snprintf(out, cap, "%s: %s",
                            mdl_result_name(r), mdl_result_default_text(r));
}
