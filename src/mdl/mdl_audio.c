...
static bool play_and_track_sound( const char *file_path ) {
	...
	if ( slot == -1 ) {
		LOG_WARN( "audio", "No free sound slots available" );
		return false;
	}
...
static bool mdl_audio_play_random_sound( const char *sound_path ) {
	...
	LOG_DEBUGF( "audio", "Searching for random variants of '%s' (base: '%s', ext: '%s')",
				sound_path, base, ext );
	...
	for ( int try_num = 0; try_num < attempts; try_num++ ) {
		...
		for ( int i = 0; i < g_num_search_paths; i++ ) {
			...
			if ( play_and_track_sound( full_path ) ) {
				LOG_DEBUGF( "audio", "Playing random variant #%d: '%s'", num, full_path );
				return true;
			}
			...
			if ( wav_upper ) {
				...
				if ( play_and_track_sound( full_path ) ) {
					LOG_DEBUGF( "audio", "Playing random variant #%d (lowercase): '%s'", num, full_path );
					return true;
				}
			}
		}
	...
	LOG_DEBUGF( "audio", "No numbered variants found, trying original filename: '%s'", sound_path );
	...
	for ( int i = 0; i < g_num_search_paths; i++ ) {
		...
		if ( play_and_track_sound( full_path ) ) {
			LOG_DEBUGF( "audio", "Playing original file: '%s'", full_path );
			return true;
		}
		...
		if ( wav_upper ) {
			memcpy( wav_upper, ".wav", 4 );
			if ( play_and_track_sound( full_path ) ) {
				LOG_DEBUGF( "audio", "Playing original file (lowercase): '%s'", full_path );
				return true;
			}
		}
	}
	...
	LOG_WARNF( "audio", "Sound not found: '%s' (tried variants and original)", sound_path );
	return false;
}
...
bool mdl_audio_init( void ) {
	...
	LOG_INFO( "audio", "Audio engine initialized successfully!" );
...
void mdl_audio_shutdown( void ) {
	...
	LOG_INFO( "audio", "Audio Engine shutdown." );
}
...
void mdl_audio_toggle_enabled( void ) {
	...
	LOG_INFOF( "audio", "Event sounds %s", g_audio_enabled ? "ENABLED" : "MUTED" );
}
...
void mdl_audio_stop_all_sounds( void ) {
	...
	LOG_INFO( "audio", "Stopped all sounds" );
}
...
void mdl_audio_set_sound_directory( const char *sound_dir ) {
	...
	if ( sound_dir == NULL || sound_dir[0] == '\0' ) {
		LOG_ERROR( "audio", "Invalid sound directory path" );
		return;
	}

	// Checking if the directory exists
	if ( !mdl_audio_directory_exists( sound_dir ) ) {
		LOG_ERRORF( "audio", "Directory does not exist: '%s'", sound_dir );
		return;
	}
	...
	LOG_INFOF( "audio", "Added sound directory (priority): %s", sound_dir );
}
...
bool mdl_audio_play_event_sound( const char *relative_path ) {
	...
	if ( !g_audio_initialized ) {
		LOG_ERROR( "audio", "Cannot play sound: engine not initialized." );
		return false;
	}
	...
	for ( int i = 0; i < g_num_search_paths; i++ ) {
		...
		if ( play_and_track_sound( full_path ) ) {
			LOG_DEBUGF( "audio", "Playing sound: '%s'", full_path );
		...
		if ( wav_check ) {
			...
			if ( play_and_track_sound( full_path_lower ) ) {
				LOG_DEBUGF( "audio", "Playing sound: '%s'", full_path_lower );
	...
	LOG_WARNF( "audio", "Sound not found: '%s'", relative_path );
	return false;
}
...
void mdl_audio_configure_for_model( const char *model_path ) {
	...
	*/
	...
	LOG_INFOF( "audio", "Configuring for model: %s", model_path );
	LOG_DEBUGF( "audio", "Searching algorithm starts from position: %s", model_dir );
	...
	for ( int level = 0; level < 10; level++ ) {
		...
		if ( has_models && ( has_sound || has_sounds ) ) {
			LOG_INFOF( "audio", "Found root directory: %s", current_dir );
			...
			if ( has_sound && g_num_search_paths < MAX_SEARCH_PATHS ) {
				...
				LOG_INFOF( "audio", "Added: %s", sound_check );
				g_num_search_paths++;
			}
			...
			if ( has_sounds && g_num_search_paths < MAX_SEARCH_PATHS ) {
				strncpy( g_sound_search_paths[g_num_search_paths], sounds_check,
						 sizeof( g_sound_search_paths[g_num_search_paths] ) - 1 );
				g_sound_search_paths[g_num_search_paths][sizeof( g_sound_search_paths[g_num_search_paths] ) - 1] = '\0';
				LOG_INFOF( "audio", "Added: %s", sounds_check );
				g_num_search_paths++;
			}
				...
				*dir_sep = '\0';
				...
				if ( mdl_audio_directory_exists( fallback_dir ) && g_num_search_paths < MAX_SEARCH_PATHS ) {
					strncpy( g_sound_search_paths[g_num_search_paths], fallback_dir,
							 sizeof( g_sound_search_paths[g_num_search_paths] ) - 1 );
					g_sound_search_paths[g_num_search_paths][sizeof( g_sound_search_paths[g_num_search_paths] ) - 1] = '\0';
					LOG_INFOF( "audio", "Added Fallback: %s", fallback_dir );
					g_num_search_paths++;
				}
				...
				LOG_INFOF( "audio", "Configured %d sound path(s)", g_num_search_paths );
	...
	LOG_WARN( "audio", "Could not auto-detect sound directory" );
	LOG_WARN( "audio", "Searched up to 10 levels from model location" );
	LOG_WARN( "audio", "Use mdl_audio_set_sound_directory() to specify manually" );
}
