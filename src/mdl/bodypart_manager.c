/*======================================================================
 *  File: bodypart_manager.c
 *  Description: Implementation of bodypart selection for Half-Life models
 *======================================================================*/
...
int bodypart_get_model_index(int bodypart_index) {
    if (!g_header || !g_data) return 0;
    if (bodypart_index < 0 || bodypart_index >= g_header->numbodyparts) return 0;
...
void bodypart_next_submodel(void) {
    if (!g_header || !g_data) return;
    if (g_bodypart_state.current_bodypart >= g_header->numbodyparts) return;
    
    mstudiobodyparts_t *bodyparts = (mstudiobodyparts_t *)(g_data + g_header->bodypartindex);
    mstudiobodyparts_t *bp = &bodyparts[g_bodypart_state.current_bodypart];
    
    if (bp->nummodels <= 1) return; // No variations to cycle through
...
bool bodypart_should_render_model(int bodypart_index, int model_index) {
    if (!g_header || !g_data) return false;
    if (bodypart_index < 0 || bodypart_index >= g_header->numbodyparts) return false;
