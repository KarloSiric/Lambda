		...
		if ( is_fan ) {
			...
			for ( int i = 2; i < num_verts; i++ ) {
				if ( vertex_count + 3 <= max_vertices ) {
					int indices[3] = { vertices_data[0].vertindex, vertices_data[i - 1].vertindex, vertices_data[i].vertindex };
...
mdl_result_t load_sequence_groups( const char *model_path, studiohdr_t *header, unsigned char *main_data, mdl_seqgroup_blob_t **groups_out, int *num_groups_out ) {
	...
	for ( int i = 1; i < num_groups; i++ ) {
		...
		// NOTE(Karlo): TO store the directory name that contains all the models
		char dir_path[256] = { 0 };
		...
		const char *filename = strrchr( sq->name, '/' );
		if ( filename ) {
			filename++; // Skip the '/' character
		} else {
			// Try backslash (Windows)
			filename = strrchr( sq->name, '\\' );
			if ( filename ) {
				filename++; // Skip the '\' character
			} else {
				// No path separator, it's already just a filename
				filename = sq->name;
			}
		}
