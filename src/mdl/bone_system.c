 ...
 *  Last Modified: 2025-09-27 13:10:26
 *----------------------------------------------------------------------
 *  Description:
 *
 *----------------------------------------------------------------------
...
static inline void Mat4Copy(mat4 src, mat4 dst) { glm_mat4_copy(src, dst); }
...
void SetUpBones(studiohdr_t *header, unsigned char *data) {
  ...
  for (int i = 0; i < header->numbones; i++) {
    ...
    if (bones[i].parent >= 0) {
      R_ConcatTransforms(g_bonetransformations[bones[i].parent], local,
			 g_bonetransformations[i]);
